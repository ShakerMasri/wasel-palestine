...
export class IncidentsService {
  ...
  async create(data: any, userId: number) {
    // ÏºÏ¦Ï¬+éÏ¿+ä+åÏº Ïº+ä+Ç userId +ç+ê+å
  ...
  async update(id: number, updateData: any) {
    ...
    if (!incident) {
      throw new Error('Ïº+äÏ¿+äÏºÏ¦ Ï¦+èÏ¦ +à+êÏ¼+êÏ»');
    }
  ...
  async remove(id: number) {
    ...
    if (!incident) {
      throw new Error('Ï¦+ü+êÏº+ïÏî +çÏºÏ» Ïº+äÏ¿+äÏºÏ¦ +àÏ¦ +à+êÏ¼+êÏ» ÏúÏÁ+äÏº+ï Ï¦Ï¦Ïº+å ÏúÏ¡Ï¦+ü+ç!');
    }

    await this.incidentRepo.remove(incident);

    return {
      message: `Ï¬+à Ï¡Ï¦+ü Ïº+äÏ¿+äÏºÏ¦ Ï¦+é+à ${id} Ï¿+åÏ¼ÏºÏ¡ +à+å Ïº+ä+åÏ©Ïº+à`,
      deletedId: id,
    };
  }
